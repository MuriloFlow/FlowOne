-- FLOW — Atestados e Equipe (quadro do dia + ocorrências)
-- Rodar no Supabase do FLOW (não no Card+).
--
-- Quadro: uma linha por loja/data com as 5 quantidades daquele dia. Sem nomes.
-- Ocorrências: atestado, falta, falta justificada ou banco de horas
-- por colaborador/unidade/dia.
-- cardplus_store_id = stores.id do Card+.
-- cardplus_collaborator_id = collaborators.id do Card+. Sem FK física.
...
create index if not exists flow_attendance_events_store_date_idx
  on public.flow_attendance_events (cardplus_store_id, date_key);

create index if not exists flow_attendance_events_person_idx
  on public.flow_attendance_events (cardplus_collaborator_id, date_key);

comment on table public.flow_attendance_events is
  'Ocorrência do dia (atestado, falta, falta justificada, banco de horas) por colaborador.';

-- Se a tabela de eventos já existia com só ATESTADO/FALTA, amplia os 4 tipos.
